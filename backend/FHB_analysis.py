...
class FastHB_CBC:
    ...
    def __init__(self, X, choice_mask, num_samples=8000, burn_in=2000, device=None):
        ...
        # Initialize Sigma with regularization to ensure positive definiteness
        self.Sigma = torch.eye(self.K, device=self.device) + 1e-3 * torch.eye(self.K, device=self.device)
    ...
    def update_beta_i(self):
        ...
        # Ensure Sigma is positive definite before creating MultivariateNormal
        Sigma_reg = self.Sigma + 1e-6 * torch.eye(self.K, device=self.device)
        mvn = torch.distributions.MultivariateNormal(self.mu, Sigma_reg)
    ...
    def update_population(self):
        ...
        # Update Sigma with regularization
        ...
        try:
            ...
            V_reg = V_posterior + 1e-6 * torch.eye(self.K, device=self.device)
            L_V = torch.linalg.cholesky(V_reg)
            ...
            # Add final regularization to ensure positive definiteness
            self.Sigma = self.Sigma + 1e-6 * torch.eye(self.K, device=self.device)
            
        except Exception as e:
            # Fallback if Cholesky fails - use more aggressive regularization
            print(f"[WARNING] Cholesky failed, using fallback: {e}")
